...
export default function DonasiPageClient() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err?.message || err?.error || `Gagal membuat QRIS (HTTP ${res.status})`);
      }
  ...
  const handleCancel = async () => {
    if (!transactionId || status === "paid" || status === "cancel" || status === "expired") return;

    const confirmed = window.confirm("Yakin ingin membatalkan pembayaran ini?");
    if (!confirmed) return;
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.message || data?.error || `Gagal membatalkan transaksi (HTTP ${res.status})`);
      }

      const canceledStatus = String(data?.data?.status || "cancel").toLowerCase();
            ...
            <div className="flex items-center justify-between bg-slate-900/30 backdrop-blur-sm rounded-2xl px-6 py-4 border border-slate-800/50">
              ...
              <div className="flex flex-wrap items-center justify-end gap-3">
                <motion.button
                  ...
                  disabled={loading || status === "paid" || status === "cancel" || status === "expired"}
                  className="rounded-xl border border-rose-500/40 bg-rose-500/10 px-5 py-3 text-sm font-medium text-rose-300 hover:bg-rose-500/20 transition disabled:cursor-not-allowed disabled:opacity-40"
                >
                  {status === "cancel" ? "Pembayaran Dibatalkan" : "Batalkan Pembayaran"}
                </motion.button>
