...
export default function QRCodeCard({ qrString, transactionId, totalAmount, expiredAt, status }: Props) {
  ...
  const copyTransactionId = async () => {
    try {
      ...
      // Fallback untuk browser yang tidak menyediakan Clipboard API.
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl bg-slate-800/30 p-5 border border-slate-700/50 backdrop-blur-sm">
              <div className="flex items-center justify-between gap-3 mb-2">
                <dt className="text-xs uppercase tracking-wider text-slate-500">Transaction ID</dt>
                <button
                  type="button"
                  onClick={copyTransactionId}
                  className="shrink-0 rounded-lg border border-slate-600/60 bg-slate-900/60 px-3 py-1.5 text-xs font-semibold text-slate-200 transition hover:bg-slate-700/70 active:scale-95"
                  aria-label="Salin Transaction ID"
                >
                  {copied ? "Tersalin ✓" : "Salin"}
                </button>
              </div>
              <dd className="text-sm font-mono text-slate-200 break-all select-text">{transactionId}</dd>
            </div>
